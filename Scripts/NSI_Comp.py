...
def read_bhdp(mergedir):
    df = pd.read_csv(mergedir+"\\MASTER_ALL_parsed.csv", parse_dates=True)
    ...
    for survey in surveylist:
        ...
        collist = surveycols + ['edi', "start_date", "SurveyName", "parent_dmis_key_name", "child_dmis_key_name",
                                "child_dmis_key_code",'date_of_birth','end_date']
        subdf = df[collist]
        subdf = subdf.dropna(how='all', subset=surveycols)
        if survey == "NSI:":
            edilist = subdf
        else:
            edilist = pd.merge(edilist,subdf, how = 'left', on = ['edi', "start_date","end_date","date_of_birth","parent_dmis_key_name","child_dmis_key_name","child_dmis_key_code"])
        i+=1
        print(len(edilist))
    print(edilist)
    ...
    dmiszips = dmiszips[['DMIS ID', 'Facility 5-Digit ZIP Code', "Facility FIPS Country Code","Facility State Code","Facility City Name"]]
...
def scoring_1(nsi, cols):
    nsi["GAD_Score"] = nsi[['GAD 7:Q3_V1', 'GAD 7:Q4_V1', 'GAD 7:Q5_V1', 'GAD 7:Q6_V1',
           'GAD 7:Q7_V1','GAD 2:Q1_V1','GAD 2:Q2_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    
    nsi["HIT6_Score"] = nsi[['HIT:Irritated_V1', 'HIT:Limit_Activities_V1', 'HIT:Pain_Severity_V1', 'HIT:Too_Tired_V1',
                             'HIT:Trouble_Concentrate_V1', 'HIT:Wish_to_Lie_Down_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    
    nsi['NSI_TotalScore'] = nsi[['NSI:Anxious_V1', 'NSI:Appetite_Change_V1', 'NSI:Balance_V1', 'NSI:Clumsy_V1', 'NSI:Decision_Difficulty_V1', 'NSI:Depressed_V1', 'NSI:Distracted_V1', 'NSI:Dizzy_V1', 'NSI:Fatigue_V1', 'NSI:Forgetfulness_V1',
                                 'NSI:Headaches_V1', 'NSI:Hearing_Difficulty', 'NSI:Irritability_V1', 'NSI:Light_Sensitivity_V1', 'NSI:Nausea_V1','NSI:Noise_Sensitivity_V1', 'NSI:Numbness_V1', 'NSI:Overwhelmed_V1',
                                 'NSI:Sleep_Difficulty_V1', 'NSI:Slow_Thinking_V1', 'NSI:Taste_Smell_Change_V1', 'NSI:Vision_Problems_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    
    nsi['PHQ9_Score'] = nsi[['PHQ9.APPETITE_V1', 'PHQ9.DEPRESSED_V1', 'PHQ9.FEEL_BAD_V1', 'PHQ9.FEEL_TIRED_V1', 'PHQ9.MOVE_SLOWLY_V1', 'PHQ9.NO_INTEREST_V1',
                             'PHQ9.TROUBLE_CONCENTRATING_V1', 'PHQ9.TROUBLE_SLEEPING_V1', 'PHQ9:Q9_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    
    nsi['AUDIT_Score'] = nsi[['AUDIT-C.HOW_MANY_DRINKS_V1', 'AUDIT-C.HOW_OFTEN_DRINK_V1', 'AUDIT-C.SIX_OR_MORE_DRINK_V1', 'AUDIT:Q10_V1', 'AUDIT:Q4_V1', 'AUDIT:Q5_V1',
                              'AUDIT:Q6_V1', 'AUDIT:Q7_V1', 'AUDIT:Q8_V1', 'AUDIT:Q9_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    
    nsi['ISI_Score'] = nsi[[x for x in cols if "ISI" in x]].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    
    nsi['pcl520_tot_score'] = nsi[[x for x in cols if "PCL" in x]].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    return nsi
...
def whoqol_scoring(nsi):
    ...
    d1 = nsi[['QOL:Pain_Limitations_V1','QOL:Medical_Treatment_V1','QOLL:Energy_Level_V1','QOL:Mobility_V1','QOL:Sleep_Satisfaction_V1','QOL:Work_Capacity_Satisfaction_V1','QOL:Daily_Activities_Satisfaction_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    d2 = nsi[['QOL:Life_Enjoyment_V1','QOL:Meaningful_Life_V1','QOL:Concentration_V1','QOL:Accept_Bodily_Appearance_V1', 'QOL:Negative_Feelings_Frequency_V1','QOL:Ability_Satisfaction_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    d3 = nsi[['QOL:Relationship_Satisfaction_V1','QOL:Sex_Life_Satisfaction_V1','QOL:Friend_Satisfaction_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    d4 = nsi[['QOL:Feel_Safe_V1','QOL:Healthy_Physcial_Environmnet_V1','QOL:Money_to_Meet_Needs_V1','QOL:Information_to_Meet_Needs_V1','QOL:Leisure_Activities_Opportunity_V1','QOL:Living_Condition_Satisfaction_V1',
              'QOL:Health_Service_Access_V1', 'QOL:Transporation_Satisfaction_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
...
def p(x):
    if x >7 : return 0
    elif x <=7 and x>=6 : return 1
    elif x <6 and x>=5 :  return 2
    elif x<5: return 3
    else: return np.nan
### Thresholds for PSQI Component 4
def e(x):
    if x >=0.85 : return 0
    elif x <0.85 and x>=0.75 : return 1
    elif x <0.75 and x>=0.65 :  return 2
    elif x<0.65: return 3
    else: return np.nan
### Thresholds for PSQI Component 5
def q(x):
    if x ==0 : return 0
    elif x >0 and x<=9 : return 1
    elif x >9 and x<=18 :  return 2
    elif x>18: return 3
    else: return np.nan
...
def psqi_scoring(nsi):
    ...
    ## Component 4
    C4 = pd.to_numeric(nsi['PSQI:Hours_of_Sleep_V1'], errors = "coerce").replace(888, np.nan)*100/(2400 - (pd.to_numeric(nsi['PSQI:Bed_Time_V1'], errors = 'coerce').replace(888, np.nan) - pd.to_numeric(nsi['PSQI:WakeUp_Time_V1'], errors = 'coerce').replace(888, np.nan)))
    C4 = C4.apply(e)
    ## Component 5
    C5cols =  ['PSQI:WakeUp_at_Night_V1','PSQI:Bathroom_at_Night_V1','PSQI:Cannot_Breathe_Comfortably_V1','PSQI:Cough_or_Snore_V1','PSQI:Feel_Too_Cold_V1','PSQI:Feel_Too_Hot_V1','PSQI:Bad_Dreams_V1','PSQI:Pain_V1','PSQI:Other_Reasons_V1']
...
def add_nicoe(nsi, nicoe):
    nicoe['NSI_TotalScore'] = nicoe[['NSI:Anxious_V1', 'NSI:Appetite_Change_V1', 'NSI:Balance_V1', 'NSI:Clumsy_V1', 'NSI:Decision_Difficulty_V1', 'NSI:Depressed_V1', 'NSI:Distracted_V1', 'NSI:Dizzy_V1', 'NSI:Fatigue_V1', 'NSI:Forgetfulness_V1',
                                 'NSI:Headaches_V1', 'NSI:Hearing_Difficulty', 'NSI:Irritability_V1', 'NSI:Light_Sensitivity_V1', 'NSI:Nausea_V1','NSI:Noise_Sensitivity_V1', 'NSI:Numbness_V1', 'NSI:Overwhelmed_V1',
                                 'NSI:Sleep_Difficulty_V1', 'NSI:Slow_Thinking_V1', 'NSI:Taste_Smell_Change_V1', 'NSI:Vision_Problems_V1']].apply(pd.to_numeric, errors = 'coerce').replace(888, np.nan).sum(axis=1, skipna = False)
    ...
    nsi['parent_dmis_key_name'] = np.where(pd.isnull(nsi['parent_dmis_key_name']), nsi['clinic'], nsi['parent_dmis_key_name'])
...
def validity_10(nsi):
    for dmis in list(nsi['parent_dmis_key_name'].unique()):
        ...
        if len(subdf)>10:
            print(dmis.replace(" ","_"), str((subdf["NSI_Validity_10"][subdf["NSI_Validity_10"]>=22].count()/len(subdf)*100).round(2))+"%" , subdf["NSI_Validity_10"][subdf["NSI_Validity_10"]>=22].count(),len(subdf))
...
def get_dmis_tables(totscores, nsi):
    ...
    for name, comps in totscores.items():
        ...
        for combo in comps:
            ...
            try:
                slope, intercept, rvalue, pvalue, stderr = linregress(pd.to_numeric(nsi[combo[0]], errors='coerce')[mask],
            ...
            if len(subdf) > 10:
                ...
                for combo in comps:
                    mask = ~np.isnan(pd.to_numeric(nsi[combo[0]], errors='coerce')) & ~np.isnan(
                        pd.to_numeric(nsi[combo[1]], errors='coerce'))
                    try:
                        slope, intercept, rvalue, pvalue, stderr = linregress(pd.to_numeric(subdf[combo[0]], errors='coerce')[mask],
...
if __name__=="__main__":
    ...
    totscores = {
     "HIT-6":   [("NSI_TotalScore","HIT6_Score"), ('NSI_F1_Vestibular',"HIT6_Score"),('NSI_F2_Cognative',"HIT6_Score"),('NSI_F3_Affective',"HIT6_Score"),('NSI_F4_Sensory',"HIT6_Score"), ('NSI:Headaches_V1', "HIT6_Score")],
      "PHQ9":  [("NSI_TotalScore","PHQ9_Score"), ('NSI_F1_Vestibular',"PHQ9_Score"),('NSI_F2_Cognative',"PHQ9_Score"),('NSI_F3_Affective',"PHQ9_Score"),('NSI_F4_Sensory',"PHQ9_Score"), ("NSI:Depressed_V1", "PHQ9_Score")],
    "AUDIT": [("NSI_TotalScore",'AUDIT_Score'), ('NSI_F1_Vestibular','AUDIT_Score'), ('NSI_F2_Cognative','AUDIT_Score'), ('NSI_F3_Affective','AUDIT_Score'), ('NSI_F4_Sensory','AUDIT_Score'), ("NSI:Depressed_V1", "AUDIT_Score")],
    "PSQI":[("NSI_TotalScore","PSQI_Score"), ('NSI_F1_Vestibular',"PSQI_Score"),('NSI_F2_Cognative',"PSQI_Score"),('NSI_F3_Affective',"PSQI_Score"),('NSI_F4_Sensory',"PSQI_Score"), ('NSI:Sleep_Difficulty_V1', "PSQI_Score")],
    "ISI":[("NSI_TotalScore",'ISI_Score'), ('NSI_F1_Vestibular','ISI_Score'), ('NSI_F2_Cognative','ISI_Score'), ('NSI_F3_Affective','ISI_Score'), ('NSI_F4_Sensory','ISI_Score'), ('NSI:Sleep_Difficulty_V1', "ISI_Score")],
    "GAD-7": [("NSI_TotalScore",'GAD_Score'), ('NSI_F1_Vestibular','GAD_Score'),('NSI_F2_Cognative','GAD_Score'),('NSI_F3_Affective','GAD_Score'),('NSI_F4_Sensory','GAD_Score'),('NSI:Anxious_V1',"GAD_Score")],
    "ABC":[("NSI_TotalScore", 'abc_tot_score'), ('NSI_F1_Vestibular','abc_tot_score'),('NSI_F2_Cognative','abc_tot_score'),('NSI_F3_Affective','abc_tot_score'),('NSI_F4_Sensory','abc_tot_score'), ('NSI:Balance_V1','abc_tot_score')],
    "DHI":[("NSI_TotalScore", 'dhi_tot_score'), ('NSI_F1_Vestibular','dhi_tot_score'), ('NSI_F2_Cognative','dhi_tot_score'), ('NSI_F3_Affective','dhi_tot_score'), ('NSI_F4_Sensory','dhi_tot_score'), ('NSI:Dizzy_V1','dhi_tot_score')],
    "Epworth":[("NSI_TotalScore", 'epworth_tot_score'),('NSI_F1_Vestibular','epworth_tot_score'),('NSI_F2_Cognative','epworth_tot_score'),('NSI_F3_Affective','epworth_tot_score'),('NSI_F4_Sensory','epworth_tot_score'),('NSI:Fatigue_V1', "epworth_tot_score")],
    "PCL520":[("NSI_TotalScore", 'pcl520_tot_score'),('NSI_F1_Vestibular','pcl520_tot_score'),('NSI_F2_Cognative','pcl520_tot_score'),('NSI_F3_Affective','pcl520_tot_score'),('NSI_F4_Sensory','pcl520_tot_score')],
    "PCL-M":[("NSI_TotalScore", 'pclm_tot_score'),('NSI_F1_Vestibular','pclm_tot_score'),('NSI_F2_Cognative','pclm_tot_score'),('NSI_F3_Affective','pclm_tot_score'),('NSI_F4_Sensory','pclm_tot_score')],
    "WHOQOL":[("WHOQOL_Physical_Score","NSI_TotalScore" ),("WHOQOL_Psychological_Score","NSI_TotalScore" ),("WHOQOL_Social_Relationships_Score","NSI_TotalScore" ),("WHOQOL_Enviornment_Score","NSI_TotalScore" )]
    }
    ...
    nicoe = pd.read_csv(r"\\wrnmdfpisismbd1\DeptShares$\Dept6\NICoE_Informatics\NICoE_Informatics\Ande\Analysis\NSI_Comp\NICOE_data_limitedCols.csv")
